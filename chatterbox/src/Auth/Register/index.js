  ...
  render() {
    return (
      <div className="Register">
        <h1 className="heading-title heading-step">{this.state.heading[this.state.stage-1].title}</h1>
	    ...
	    <Button onClick={()=>this.handleNext(this.state.stage)} raised className="button-next">
	      NEXT
	    </Button>
	    {this.state.stage!=1 && <div onClick={this.handleSkip.bind(this)} className="skip">Skip for now</div>}
	    {this.state.error && <div className="error-display">{this.state.heading[this.state.stage-1].error}</div>}
      </div>
    );
  }
}
